...
_factionToSpawn = "";
switch (_faction) do {
	case "North": {_factionToSpawn = lmn_US; _side = west};
	case "USA": {_factionToSpawn = lmn_pavn};
	case "ROK": {_factionToSpawn = lmn_pavn};
	case "AUS": {_factionToSpawn = lmn_pavn};
	case "NZ": {_factionToSpawn = lmn_pavn};
};
...
_prep setTriggerStatements [
	"this",
	"[thisTrigger] remoteExec ['lmn_fnc_spawnProbe', 2]",
