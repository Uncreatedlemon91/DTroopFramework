...
// Loop the locations
{
	...
	_nearTrig = selectRandom (nearestObjects [position _trig, ["EmptyDetector"], 1500, true]);

	// Grab near location's details 
	_trigTroopLevel = _nearTrig getVariable "TroopCount";
	_trigMaxTroopLevel = _nearTrig getVariable "MaxTroopCount";
	_trigFaction = _nearTrig getVariable "Faction";
	...
	if (_faction == _trigFaction) then {
		...
		if ((_troopLevel > _trigTroopLevel) AND (_trigTroopLevel < _trigMaxTroopLevel) AND (_troopLevel > 30)) exitwith {
			[_trig, _nearTrig] remoteExec ["lmn_fnc_ldSendTroops", 2];
	...
	if (_faction != _trigFaction) then {
		...
		if (_troopLevel > _trigTroopLevel) AND (_troopLevel > 40) exitWith {
			[_trig, _nearTrig] remoteExec ["lmn_fnc_ldAttackSite", 2];
